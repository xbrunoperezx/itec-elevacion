...
include("conn_bbdd.php");
  ...
  if(isset($_POST['id']) && $_POST['id'] !== ''){
    ...
    $venc = ($vencimiento=='' || $vencimiento==null) ? '0000-00-00' : $vencimiento;
    $sql = "INSERT INTO clientes (rae,nombre,direccion,localidad,municipio,cp,provincia,quien_contrata,observaciones,id_mantenedor,vencimiento,cada,id_administrador,telefono,telefono2,email) VALUES ('{$rae}','{$nombre}','{$direccion}','{$localidad}','{$municipio}','{$cp}','{$provincia}','{$quien_contrata}','{$observaciones}','{$id_mantenedor}','{$venc}','{$cada}','{$id_administrador}','{$telefono}','{$telefono2}','{$email}')";

    if (mysqli_query($link, $sql)) {
        echo "OK";
    } else {
        echo "Error al insertar nuevo cliente: " . mysqli_error($link);
    }
  }
