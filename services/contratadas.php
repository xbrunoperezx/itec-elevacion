...
if($filtro_id_cliente!=""){
    $sql = "SELECT c.*, con.id AS con_id, con.fecha, con.id_usuarios, con.estado, con.num_control, con.observaciones AS con_observaciones, con.nocobrar, con.precio, con.id_formas_pago AS con_id_formas_pago, con.id_tarifa AS con_id_tarifa, con.enviada_cobrar FROM clientes c JOIN contratadas con ON c.id = con.id_cliente WHERE con.id_cliente=" . $filtro_id_cliente;
}else if($id==""){
    // Define la consulta SQL
    $sql = "SELECT c.*, con.id AS con_id, con.fecha, con.id_usuarios, con.estado, con.num_control, con.observaciones AS con_observaciones, con.nocobrar, con.precio, con.id_formas_pago AS con_id_formas_pago, con.id_tarifa AS con_id_tarifa, con.enviada_cobrar FROM clientes c JOIN contratadas con ON c.id = con.id_cliente";
...
while ($row = mysqli_fetch_assoc($result)) {
    ...
    $contratada = array();
    foreach (array_keys($row) as $key) {
        if (in_array($key, ["con_id","id_cliente","fecha","id_usuarios","estado","num_control","con_observaciones","nocobrar","precio","con_id_formas_pago","con_id_tarifa","enviada_cobrar"])) {
            if($key=="id_usuarios"){
